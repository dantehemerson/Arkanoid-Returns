...
void About::update() {
	...
	/*Si el mouse cambio de posici�n entonces el usuario quiere usar el mouse entonces
...
void About::doAction(action_t action, int magnitute) {
	buttons.at(index)->isFocused(false);
	switch (action) {
		/* Las teclas UP y LEFT tienen la misma funci�n. */
	case ActionMenu::UP:
	case ActionMenu::LEFT:
		/*Si el foco estuvo en el primer bot�n entonces el foco se ubica en el �ltimo bot�n.*/
		index = (index + buttons.size() - 1) % buttons.size();
		useMouse = false;
		break;

		/* Ac� igualmente estas dos teclas tienen la misma funci�n. */
